...
TotemPowerWrathEraWidgetsGridviewMixin = {}
function TotemPowerWrathEraWidgetsGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
...
function TotemPowerWrathEraWidgetsGridviewMixin:InitFramePool(type, template)
    self.framePool = CreateFramePool(type, self.scrollChild, template);
end

function TotemPowerWrathEraWidgetsGridviewMixin:SetMinMaxSize(min, max)
    self.itemMinWidth = min;
    self.itemMaxWidth = max;
end

function TotemPowerWrathEraWidgetsGridviewMixin:SetFixedColumnCount(count)
    self.fixedColumnCount = count;
end

function TotemPowerWrathEraWidgetsGridviewMixin:InsertCustomFrame(frame)
    if self.frames then
...
function TotemPowerWrathEraWidgetsGridviewMixin:Insert(info)
...
function TotemPowerWrathEraWidgetsGridviewMixin:RemoveFrame(frame)
    local key;
...
function TotemPowerWrathEraWidgetsGridviewMixin:InsertTable(tbl)

end

function TotemPowerWrathEraWidgetsGridviewMixin:Flush()
    self.data = {}
    for k, f in ipairs(self.frames) do
        if f.ResetDataBinding then
            f:ResetDataBinding()
...
function TotemPowerWrathEraWidgetsGridviewMixin:GetItemSize()
...
function TotemPowerWrathEraWidgetsGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function TotemPowerWrathEraWidgetsGridviewMixin:GetFrames()
    return self.frames;
end